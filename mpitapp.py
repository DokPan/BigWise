...
class TaxiPriceAssistant:
    ...
    def show_csv_preview(self, results_df):
        ...
        display_df['is_done'] = display_df['accept_probability'].apply(
            lambda x: 'done' if x >= 0.6 else 'cancel'
        )
        ...
        print(display_df[['order_id', 'passenger_price', 'recommended_price',
                         'price_increase_percent', 'is_done']].to_string(index=False))
    ...
    def save_csv_results(self, results_df, original_filename):
        ...
        output_df['is_done'] = output_df['accept_probability'].apply(
            lambda x: 'done' if x >= 0.6 else 'cancel'
        )
    
        output_df = output_df.drop('accept_probability', axis=1)
    
        output_df['price_increase_percent'] = output_df['price_increase_percent'].apply(lambda x: f'{x:+.1f}%')

        final_columns = ['order_id', 'passenger_price', 'recommended_price', 'price_increase_percent', 'is_done']
        ...
        done_count = (output_df['is_done'] == 'done').sum()
        cancel_count = (output_df['is_done'] == 'cancel').sum()
        print(f"Рекомендуется принять: {done_count} заказов ({done_count/len(output_df)*100:.1f}%)")
        print(f"Рекомендуется отклонить: {cancel_count} заказов ({cancel_count/len(output_df)*100:.1f}%)")
    ...
    def save_single_order(self, recommendation):
        ...
        single_df['is_done'] = 'done' if recommendation['accept_probability'] >= 0.6 else 'cancel'
        single_df = single_df.drop('accept_probability', axis=1)
        ...
        print(f"Рекомендация: {single_df['is_done'].iloc[0]}")
