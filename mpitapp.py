...
class PriceRecommender:
    ...
    def ab_test_analysis(self, historical_data):
        """Главная функция для A/B тестирования"""
        results = []
...
class TaxiPriceAssistant:
    ...
    def load_model(self):
        ...
        try:
            ...
            # Создаем рекомендатор и загружаем в него данные
            ...
            # Загружаем модель вероятности если есть
    ...
    def preprocess_order_data(self, order):
        # Добавляем недостающие признаки на основе имеющихся данных
        ...
        # Погода
        if 'season' in order and 'order_hour' in order and 'order_month' in order:
            season = order['season']
            hour = order['order_hour']
            month = order['order_month']

            # Та же логика, что и при обучении
            if season == 'winter':
                if month in [12, 1]:
                ...
                if hour < 11: weather = 'sunny'
                elif hour < 18: weather = 'cloudy'
                else: weather = 'rainy'
            elif season == 'spring':
        ...
        # Временные признаки
        if 'order_day_of_week' in order:
            order['is_weekend'] = 1 if order['order_day_of_week'] >= 5 else 0

        if 'order_hour' in order and 'is_weekend' in order and 'is_holiday' in order:
            hour = order['order_hour']
    ...
    def process_csv_file(self):
        ...
        # создаем окно для выбора нужного файла
        root = tk.Tk()
        root.withdraw()  # Скрываем главное окно
        root.attributes('-topmost', True)  # Поверх всех окон
        
        # появление диалог выбора файла
        ...
        # файл не выбран (нажали Отмена)
        if not file_path:
            print("Файл не выбран")
            return

        # имя файла неизвестно заранее 
        ...
        try:
            ...
            df_processed = self.recommender.preprocess_data(df)
    ...
    def show_detailed_recommendation(self, recommendation, order_data):
        """Детальный показ рекомендации с анализом факторов"""
    ...
    def show_csv_preview(self, results_df):
        ...
        display_df['price_increase_percent'] = display_df['price_increase_percent'].apply(lambda x: f'{x:+.1f}%')
        display_df['accept_probability'] = display_df['accept_probability'].apply(lambda x: f'{x:.1%}')

        display_df = display_df.sort_values('price_increase_percent', ascending=False)

        print(display_df[['order_id', 'passenger_price', 'recommended_price',
                         'price_increase_percent', 'accept_probability']].to_string(index=False))
    ...
    def save_csv_results(self, results_df, original_filename):
        ...
        output_df['price_increase_percent'] = output_df['price_increase_percent'].apply(lambda x: f'{x:+.1f}%')
        output_df['accept_probability'] = output_df['accept_probability'].apply(lambda x: f'{x:.1%}')
    ...
    def save_single_order(self, recommendation):
        ...
        single_df = pd.DataFrame([recommendation])
        single_df['price_increase_percent'] = single_df['price_increase_percent'].apply(lambda x: f'{x:+.1f}%')
        single_df['accept_probability'] = single_df['accept_probability'].apply(lambda x: f'{x:.1%}')

        single_df.to_csv(filename, index=False)
